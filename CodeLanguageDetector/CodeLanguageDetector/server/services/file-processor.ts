...
export class FileProcessor {
  ...
  private async processZipFile(buffer: Buffer, zipFilename: string): Promise<ProcessedFile[]> {
    ...
    for (const [path, file] of Object.entries(zipContent.files)) {
      ...
      // Process all files - we'll detect languages from both source and binary files
      ...
      try {
        ...
        // Try binary detection first for executable files
        const binaryResult = this.detector.detectFromBinary(filename, fileBuffer);
        if (binaryResult) {
          detectionResult = binaryResult;
          content = `Binary file (${detectionResult.language})`;
        } else if (this.isCodeFile(filename)) {
          // Only try text detection for actual source code files
          content = fileBuffer.toString('utf8');
          detectionResult = this.detector.detectFromContent(content, filename);
        } else {
          // Skip non-code, non-binary files
          continue;
        }
